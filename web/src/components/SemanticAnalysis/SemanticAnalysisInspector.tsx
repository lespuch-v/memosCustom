...
const InspectorBody = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between border-t pt-3"><span className="text-xs text-muted-foreground">{result.model}</span><Button variant="ghost" size="sm" onClick={refresh}><RefreshCwIcon className="mr-1 size-3.5" />{t("semantic-analysis.analyze-again")}</Button></div>
    </div>
  );
};
